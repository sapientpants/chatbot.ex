      ...
      <div class="flex items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
        <div class="w-full max-w-md">
          <div class="bg-base-100 shadow-xl rounded-2xl p-8">
            <h1 class="text-3xl font-bold text-center mb-2">Register for an account</h1>
            ...
            <.form
              ...
              class="space-y-5"
            >
              <div :if={@check_errors} class="alert alert-error shadow-lg">
                <svg
              ...
              <div class="form-control">
                ...
                <div class="relative">
                  <input
                    ...
                    class="input input-bordered w-full pr-12 focus:input-primary transition-colors"
                    required
                  />
                  <button
                    type="button"
                    phx-click="toggle_password"
                    class="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 rounded-lg text-base-content/60 hover:text-base-content hover:bg-base-200 transition-colors"
                ...
                <div class="mt-3 p-4 bg-base-200/50 rounded-lg text-sm border border-base-300">
                  <p class="font-semibold mb-2.5 text-base-content">Password requirements:</p>
                  <ul class="space-y-2">
                    <li class={[
                      "flex items-center gap-2 transition-colors",
                      password_requirement_met?(@form[:password].value, :length) &&
                        "text-success font-medium"
                    ]}>
                      <span
                        :if={password_requirement_met?(@form[:password].value, :length)}
                        class="text-lg"
                      >
                        ✓
                      </span>
                    ...
                    <li class={[
                      "flex items-center gap-2 transition-colors",
                      password_requirement_met?(@form[:password].value, :uppercase) &&
                        "text-success font-medium"
                    ]}>
                      <span
                        :if={password_requirement_met?(@form[:password].value, :uppercase)}
                        class="text-lg"
                      >
                        ✓
                      </span>
                    ...
                    <li class={[
                      "flex items-center gap-2 transition-colors",
                      password_requirement_met?(@form[:password].value, :special) &&
                        "text-success font-medium"
                    ]}>
                      <span
                        :if={password_requirement_met?(@form[:password].value, :special)}
                        class="text-lg"
                      >
                        ✓
                      </span>
              ...
              <div class="pt-2">
                <.button phx-disable-with="Creating account..." class="w-full btn-lg">
                  Create an account
                </.button>
              </div>
            </.form>
